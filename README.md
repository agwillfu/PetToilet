...
| 項目 | 狀態 |
|---|---|
| 韌體（ESP8266 + Adafruit IO） | ✅ 裝置上執行 v0.3.1 |
...
```
  SW-420 ──中斷──> NodeMCU ──TLS/MQTT──> Adafruit IO ──> iOS App
                      │
                      └──> IRF520 ──> 水泵
```
...
- **獨立的時間長度**。日常沖水（`flush-duration`，上限 120 秒）與定時清洗（`dur`，上限 300 秒）
